import { Files, Search, Settings, Package, GitBranch } from "lucide-react";
...
export function ActivityBar({ activePanel, setActivePanel }: ActivityBarProps) {
  ...
  const items = [
    ...
    { id: "git" as Panel, icon: <GitBranch size={16} />, title: "Git" },
    { id: "extensions" as Panel, icon: <Package size={16} />, title: "Extensions" },
  ];

  return (
    <div className="flex flex-row items-center h-10 bg-[#030303] border-b border-[#003a00] shrink-0 w-full gap-1" style={{ paddingLeft: '12px', paddingRight: '12px' }}>
      {items.map((item) => (
        <button
          key={item.id}
          title={item.title}
          ...
          className={`
            h-10 px-4 flex items-center justify-center gap-1.5 transition-colors relative text-sm cursor-pointer
      ...
      <button
        ...
        className={`
          h-10 px-4 flex items-center justify-center gap-1.5 transition-colors relative text-sm
          ${settingsOpen
            ? "text-[#00ff41] after:absolute after:bottom-0 after:left-2 after:right-2 after:h-[2px] after:bg-[#00ff41]"
            : "text-[#2d7a3a] hover:text-[#00ff41]"
          }
