...
#[tauri::command]
fn search_in_files(root: String, query: String, ignore: Vec<String>) -> Result<Vec<SearchMatch>, String> {
    ...
    for entry in WalkDir::new(&root)
        ...
        .filter(|e| {
            ...
            if !ignore.is_empty() {
                let ignored = e.path().components().any(|c| {
                    let s = c.as_os_str().to_string_lossy();
                    ignore.iter().any(|ig| s == ig.as_str())
                });
...
fn get_git_diff(path: String, file: String, status: String) -> Result<String, String> {
    ...
    if xy == "??" {
        ...
        if file_path.is_dir() {
            let mut lines = vec![format!("// New directory: {}", file)];
